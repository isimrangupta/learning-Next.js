...
const DatafetchServer = async (props) => {
  ...
  if (!userName) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 text-gray-800 mb-4">
  ...
  try {
    ...
    //  Check for API error or missing gender
    if (userData.error || !userData.gender) {
      return (
        <div className="min-h-screen flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-8 text-center">
            <h1 className="text-2xl font-bold mb-2">Error</h1>
            <p className="text-gray-600">
              {userData.error || "Is name ka gender data available nahi hai."}
            </p>
          </div>
        </div>
      );
  ...
  //  Original UI safe rendering
  const isMale = userData.gender === "male";
  const confidencePercentage = userData.probability * 100;

  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl p-8 max-x-sm w-full relative overflow-hidden">
        {/* BACKGROUND DECORATION */}
        <div
          className={`absolute top-0 right-0 w-32 h-32 ${
            isMale ? "bg-blue-100" : "bg-pink-100"
          } rounded-full -translate-y-16 opacity-50`}
        ></div>

        <div
          className={`absolute bottom-0 left-0 w-24 ${
            isMale ? "bg-blue-50" : "bg-pink-50"
          } rounded-full -translate-y-17 opacity-50`}
        ></div>
        ...
        <div className="relative z-10">
          ...
          <div className="text-center mb-6">
            ...
            >
              <User className="w-12 text-white" strokeWidth={1.5} />
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              {/* Accuracy Indicator */}
              <div
                className={`rounded-lg p-4 mt-4 border-2 ${
